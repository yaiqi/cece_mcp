...
def _to_number(value: object) -> int | float | object:
...
def _format_park_detail(raw: dict) -> dict:
    ...
    area_text = f"{_to_number(area)}{area_unit or ''}" if area is not None else None
    return {
        ...
        "经纬度": {"经度": _to_number(raw.get("lng")), "纬度": _to_number(raw.get("lat"))} if raw.get("lng") is not None and raw.get("lat") is not None else None,
...
def _with_unit(value: object, unit: str) -> str | None:
    if value is None:
        return None
    return f"{_to_number(value)}{unit}"
...
def _format_sub_park_list(raw: object) -> list[dict]:
    ...
    return [
        {
            ...
            "子园区内企业数量": _with_unit(item.get("companyCount"), "家"),
        }
        for item in raw
...
def _format_industry_distribution(raw: object) -> list[dict]:
    ...
    for item in raw:
        ...
        result.append({
            ...
            "占比": _with_unit(_first_non_null(item.get("ratio"), item.get("proportion"), item.get("percent")), "%"),
        })
    return result


def _format_company_count_statistics(raw: object) -> dict:
    """将企业分类统计压缩为“分类名称: 数量家”。"""
    if not isinstance(raw, dict):
        return {}
    return {
        name: _with_unit(value.get("count"), "家") if isinstance(value, dict) else _with_unit(value, "家")
        for name, value in raw.items()
    }


def _format_company_feature_distribution(raw: object) -> dict:
    """将四个企业特征维度压缩为“类别: 占比%”。"""
    if not isinstance(raw, dict):
        return {}
    dimensions = {
    ...
    for source_key, display_name in dimensions.items():
        ...
        if not isinstance(entries, list):
            continue
        result[display_name] = {
            str(item.get("name") or item.get("label") or item.get("typeName")): _with_unit(
                _first_non_null(
                    item.get("ratio"),
                    item.get("proportion"),
                    item.get("percent"),
...
def _format_park_company_list(raw: object) -> dict:
    """仅保留园区企业列表中适合回答的业务字段。"""
...
def _format_park_query_result(
    result: dict,
    success_summary: str,
    data_transform: Callable[[dict], dict] | None = None,
) -> dict:
    """统一园区查询工具的 MCP 返回契约。"""
    ...
    data = _remove_internal_identifier_fields(data)
...
async def resolve_park(query: str) -> dict:
    ...
    try:
        ...
        return call_failed("园区实体识别服务暂不可用，请稍后重试。")
    if result:
        if result.get("status") == "ambiguous":
            candidates = result.get("candidates", [])
            return multiple_candidates("园区", query, candidates) if candidates else no_match("园区", query)
        return call_failed(result.get("error_message", "园区消歧调用失败。"))
    return unique_match("园区", query, park)
...
async def park_detail(park_ref: ParkRef) -> dict:
    ...
    return _format_park_query_result(
...
async def park_sub_parks(park_ref: ParkRef) -> dict:
    ...
    return _format_park_query_result(unwrap(resp), "已查询到子园区列表。", _format_sub_park_list)
...
async def park_industry_distribution(park_ref: ParkRef) -> dict:
    ...
    return _format_park_query_result(unwrap(resp), "已查询到园区产业分布。", _format_industry_distribution)
...
async def park_company_features(park_ref: ParkRef) -> dict:
    ...
    if count_result["status"] != "success":
        return _format_park_query_result(count_result, "")
    ...
    if features_result["status"] != "success":
        return _format_park_query_result(features_result, "")
    data = {
        "企业数量统计": _format_company_count_statistics(count_result["data"]),
        "企业特征分布": _format_company_feature_distribution(features_result["data"]),
    }
    return {
        "状态": "查询成功",
        "摘要": "已查询到园区企业特征分布。",
...
async def park_companies(
    ...
    return _format_park_query_result(unwrap(resp), "已查询到园区入驻企业列表。", _format_park_company_list)
